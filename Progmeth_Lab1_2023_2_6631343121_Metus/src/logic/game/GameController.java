...
public class GameController {
    ...
    public void endDay() {
        ...
        for (Player player : players){
             int diff = player.getEnergy()-3;
             if (diff>0){
                 player.setEnergy(diff);
             }
             else {
                 try {
                     player.getStatus().setHp(Math.max(0, player.getStatus().getHp() + diff));
                     player.setEnergy(0);
    ...
    public static Status createNewStatus(int hp, int durable, int attack,int magic) {
       /*FILL CODE*/
        ...
        catch (Exception BadStatusException){return null;}
    }
}
